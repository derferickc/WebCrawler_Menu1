...
app.listen(8000, function(){
  console.log("Listening for Green Theory on port 8000");
});
...
app.get('/', function(req, res){
	...
	var result = htmlSnapshots.run({
		input:"array",
	  // source: "/menu/",
	  source: ["http://www.green-theory.com/menu"],
	  outputDir: "./snapshots",
	  // outputDirClean: true,  
	  selector: "#imp-love",
	  snapshotScript: {
				...
				// add: function(req, res) {[[]]
					...
					function insert(connection, callback){
						for(var i=0; i<strainName.length; i++){
							var post = {vendor_id: 583, strain_name: strainName[i], price_weight: strainPrice_Weight[i]};
							connection.query("INSERT INTO greentheory_menu SET ?, created_at = NOW(), updated_at = NOW()", post, function(error, reservations, fields) {
									if (error) {
										console.log(error);
									} else {
